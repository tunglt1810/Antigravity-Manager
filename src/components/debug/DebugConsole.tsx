...
const DebugConsole: React.FC<DebugConsoleProps> = ({ embedded = false }) => {
    ...
    const content = (
        <div
            className={cn(
                "flex flex-col font-sans bg-[#1e1e1e] text-gray-300",
                embedded
                    ? "h-full w-full rounded-xl border border-white/10 shadow-inner overflow-hidden"
                    : "fixed bottom-0 left-0 right-0 border-t border-zinc-800 shadow-2xl z-[9999]"
            )}
        ...
        >
            ...
            {/* Toolbar */}
            <div className={cn(
                "flex items-center justify-between px-3 py-2 bg-[#252526] border-b border-black/20 select-none",
                embedded && "rounded-t-xl"
            )}>
                <div className="flex items-center gap-3">
                    <span className="flex items-center gap-2 text-zinc-400 font-medium text-xs tracking-wide">
                            ...
                            <button
                                ...
                                className={cn(
                                    ...
                                    filter.includes(level)
                                        ? LEVEL_CONFIG[level].color + " bg-white/10 shadow-sm"
                    ...
                    <div className="relative group ml-2">
                        <Search size={13} className="absolute left-2.5 top-1.5 text-zinc-500 group-focus-within:text-zinc-300 transition-colors" />
                        <input
                            type="text"
                            value={searchTerm}
                            onChange={e => setSearchTerm(e.target.value)}
                            placeholder="Filter logs..."
                            className="bg-black/20 border border-transparent focus:border-white/10 focus:bg-black/40 rounded-md pl-8 pr-3 py-1 text-xs text-zinc-300 w-40 focus:w-64 transition-all focus:outline-none placeholder:text-zinc-600"
                        />
                    </div>
                </div>
                ...
                <div className="flex items-center gap-1.5">
                    ...
                    <button
                        onClick={clearLogs}
                        className="p-1.5 rounded-md text-zinc-500 hover:text-red-400 hover:bg-white/5 transition-all"
                    ...
                    {!embedded && (
                        <button
                            onClick={close}
                            className="p-1.5 rounded-md text-zinc-500 hover:text-white hover:bg-white/5 transition-all ml-1"
                        >
                            <X size={14} />
                        </button>
                    )}
            ...
            <div
                ...
                className={cn(
                    "flex-1 overflow-y-auto overflow-x-hidden bg-[#1e1e1e] font-mono text-xs",
                    // Custom scrollbar styling
                    "[&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-[#1e1e1e] [&::-webkit-scrollbar-thumb]:bg-[#424242] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:border-2 [&::-webkit-scrollbar-thumb]:border-[#1e1e1e] hover:[&::-webkit-scrollbar-thumb]:bg-[#4f4f4f]",
            ...
            {/* Footer */}
            <div className={cn(
                "flex items-center justify-between px-3 py-1.5 border-t border-black/20 bg-[#007acc] text-white", // VS Code blue footer style
                ...
                <div className="flex items-center gap-4">
                    ...
                    {(Object.keys(LEVEL_CONFIG) as LogLevel[]).map(level => {
                        ...
                        return (
                            <span
                                key={level}
                                className="text-[10px] font-medium flex items-center gap-1.5 select-none"
                            >
                                {React.cloneElement(LEVEL_CONFIG[level].icon as React.ReactElement, { size: 12, className: "text-white/80" })}
                                {count}
                            </span>
                        );
                    })}
                ...
                <div className="flex items-center gap-3">
                    {!autoScroll && (
                        <button
                            ...
                            className="flex items-center gap-1.5 px-2 py-0.5 rounded bg-black/20 hover:bg-black/30 text-[10px] font-medium transition-colors"
                        >
